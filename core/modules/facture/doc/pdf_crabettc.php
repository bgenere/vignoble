...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_crabettc.modules.php
 *	\ingroup    facture
 ...
 */
class pdf_crabettc extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	 ...
	 */
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=1)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$up_with_tax = pdf_getlineupwithtax($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxup, $curY);
					$pdf->MultiCell($this->posxqty-$this->posxup-0.8, 3, $up_with_tax, 0, 'R', 0);
					...
					// Total TTC line
					$total_with_tax = pdf_getlinetotalwithtax($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->postotalht, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->postotalht, 3, $total_with_tax, 0, 'R', 0);
