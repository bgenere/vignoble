...
/**
 *	\file       htdocs/core/modules/vignoble/doc/pdf_einstein.modules.php
 *	\ingroup    vignoble
 *	\brief      Fichier de la classe permettant de generer les vignobles au modele Einstein
 */

$s=dol_include_once('/vignoble/core/modules/vignoble/modules_vignoble.php');
...
/**
 *	Classe to generate PDF orders with template Plot
 */
class pdf_plotabc extends ModelePDFPlot
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "plotabc";
		$this->description = $langs->trans('PlotModele');
...
// 		$outputlangs->load("orders");
		$outputlangs->load("vignoble@vignoble");

		$nblignes = count($object->lines);

		if ($conf->vignoble->dir_output)
		{
			...
			if ($object->specimen)
			{
				$dir = $conf->vignoble->dir_output;
				$file = $dir . "/SPECIMEN.pdf";
			}
			else
			{
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->vignoble->dir_output . "/" . $objectref;
...
//var_dump($file);exit;
		...
		{
			$this->error=$langs->trans("ErrorConstantNotDefined","PLOT_OUTPUTDIR");
			return 0;
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey="Order")
	{
		...
		if($object->statut==0 && (! empty($conf->global->PLOT_DRAFT_WATERMARK)) )
		{
            pdf_watermark($pdf,$outputlangs,$this->page_hauteur,$this->page_largeur,'mm',$conf->global->PLOT_DRAFT_WATERMARK);
		}
